...
- `sinceDate` (선택): 소스별 크롤 플로어. 없으면 `SERVICE_START_DATE`(2026-03-09). **사이트가 아카이브를 일괄 이관하면 옛 글 전부가 이관일로 찍혀** 서비스 플로어보다 뒤에 오고, 그러면 플로어가 무력해져 수백 건이 한 날짜로 들어온다 — 그때만 쓴다 (saint: `2026-04-24`, `docs/known-issues.md` §14). 크롤 설정이라 `server-sources.json`에는 안 나감.
