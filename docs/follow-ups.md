...
손상 2,126건 중 **2,027건(95.3%)이 네트워크 0으로 완전 복구 가능**, 부분 12건, 불가 87건. 불가 87건은 마크다운 힌트의 URL이 현재 본문의 `<img>`와 하나도 안 겹치는 경우 — 같은 편집에서 이미지가 통째로 교체된 문서다. 복원할 것이 없는 게 맞고, 다음 tier-2 실행이 실제로 측정한다.
...
**실행 절차** (아직 안 함):
1. `skkuverse-crawler repair-dimensions` — dry run, 규모 확인
2. `skkuverse-crawler repair-dimensions --apply`
3. 확인: 손상 지문(`md`에 `{WxH}` + `html`에 `width=` 없음) 문서 수가 87 근처로 수렴
