...
### ~~8-b. tier-2가 손상시킨 문서 백필~~ ✅ (2026-08-03) — 구현 완료, 실행 대기

**문서가 틀렸다.** "재크롤이 필요하다(로컬 재계산 불가)"라고 적었으나, **측정값은 이미 DB 안에 있었다.** tier-2는 `cleanHtml`만 덮어썼고 `cleanMarkdown`은 건드리지 않았으므로, 낡은 마크다운이 `{WxH}` 힌트를 그대로 갖고 있다.

**실측 (2026-08-03, 전체 6,792건):**

| 상태 | 문서 |
|------|------|
| tier-2 미접촉 (무손상) | 4,037 |
| tier-2 접촉 + 차원 소실 | **2,126** (거의 전부 이미지 보유) |
| tier-2 접촉, 차원 정상 | 629 |

손상 2,126건 중 **2,076건이 네트워크 0으로 완전 복구 가능**, 나머지 약 37건은 마크다운 힌트의 URL이 현재 본문의 `<img>`와 하나도 안 겹치는 경우 — 같은 편집에서 이미지가 통째로 교체된 문서다. 복원할 것이 없는 게 맞고, 다음 tier-2 실행이 실제로 측정한다.

> 초안은 2,027/87이었다. 차이는 리뷰가 잡은 정규식 버그다 — 알트 텍스트가 `\[학사팀\]`처럼 이스케이프된 49건을 못 읽었고, **못 읽은 힌트는 재주입도 못 하므로 마크다운 재생성 시 영구 소실**됐다. 복구 도구가 복구 대상을 파괴하는 형태였다.
...
앞 세 스테이지만 raw HTML을 필요로 한다. 저장된 `content`에서 `cleanHtml`을 다시 만드는 것은 **금지** — 이미 정규화된 HTML을 `clean_html`에 다시 먹이면 모든 공지의 본문이 조용히 바뀐다 (adr-006 §④). 그래서 저장된 `cleanHtml`을 입력으로 쓴다.

`contentText`도 같은 경로로 고쳐진다(strategy 텍스트 → 정제 HTML 추출).

**멱등**: 이미 정합한 문서는 동일 필드를 되돌려주므로 쓰기가 일어나지 않는다. `editHistory`·`crawledAt`은 건드리지 않는다 — 파생 필드 복구이지 소스에서 변경을 관측한 게 아니므로, 둘 중 하나라도 기록하면 데이터에 거짓이 남는다.

**실행 절차** (아직 안 함):

1. `skkuverse-crawler repair-dimensions` — dry run. `repaired` / `already_consistent` / `Images with no stored measurement` 확인
2. `skkuverse-crawler repair-dimensions --apply`
3. 재실행이 `repaired: 0`이면 완료 (멱등)

⚠️ **지문 개수로 검증하지 말 것.** 손상 지문(`md`에 `{WxH}` + `html`에 `width=` 없음)은 복구 후 **0으로 수렴한다** — 복원 불가 문서도 마크다운이 현재 HTML에서 재생성되면서 낡은 힌트가 사라지기 때문이다. 0은 정상 완료와 도구 고장을 구분해주지 못한다. 판정은 **두 번째 실행의 `repaired: 0`**으로 한다.

⚠️ **크롤 크론과 겹치지 않게.** 크롤은 `*/30`이고 전체 1회 통과는 컨테이너에서 약 60초다. 읽기와 `$set` 사이에 크롤이 같은 공지를 갱신하면 복구가 옛 본문으로 되돌린다. 창이 좁지만 크롤 직후에 돌리는 게 안전하다.
