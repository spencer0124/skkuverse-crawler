...
**`VIEWS_REFRESH_JITTER_TICKS = 8`** — `articleNo % 8` **틱**(= 4시간)을 interval에 **더한다**.
...
- **단위가 '시간'이 아니라 '틱'인 이유**: 쓰기는 틱에서만 일어난다. 처음엔 `% 4` **시간**으로
  넣었는데, 그러면 코호트가 창(window) 안의 12개 틱 중 **4개에만 몰리고 나머지 8개는 비었다** —
  덩어리가 작아질 뿐 여전히 덩어리다. 크론이 `*/30`이니 offset도 30분 단위여야 한다.
  8틱 = 4시간으로 **staleness 상한(6~10h)은 그대로 두고 슬롯만 2배**로 늘렸다.
- 400개 코호트 시뮬레이션: 창 안의 **모든 틱이 코호트의 일부를 가져가고**, 어느 틱도
  1/8의 2배를 넘지 않는다. 배포 후 **한 사이클이면 herd가 영구히 흩어진다.**
