...
- **1만 건 미만 규모는 ENN(`exact: true`) 스위트스팟** — MongoDB 공식 가이드 권장. numCandidates 튜닝 없이 항상 정확한 결과. (코퍼스 건수는 크롤 주기마다 변하므로 여기 박지 않는다 — 2026-08-01 기준 6,714건)
- ~~**`dotProduct` 선택 근거**~~ → **auto-embed 에서는 유사도 함수를 우리가 지정하지 않는다.** Atlas 가 모델에 맞춰 결정한다. (Voyage 출력이 L2-normalized 인 것은 자체 측정으로 확인 — `skkuverse-ai/eval/results.md`)
- ~~**벡터 저장은 BSON BinData float32**~~ → **벡터를 우리가 저장하지 않는다** (adr-007). Atlas 내부 DB 에 `scalar`(int8) 양자화로 저장된다 — 1024차원 1,026바이트.
- **필터 일관성 규칙**: `sourceId`/`date`/`summaryType` 필터는 **두 인덱스 모두에 선언**하고 **두 서브파이프라인에 동일 적용**해야 함 ($rankFusion 사후 필터는 순위를 깨뜨림). ⚠️ `date` 사전순 범위 필터는 **현재 깨져 있다** — 개정 블록 5번 참조.
...
- **`$scoreFusion`**: `$rankFusion` 의 형제 스테이지로 원점수 기반 결합을 하지만, **이 클러스터에서 미인식**(`Unrecognized pipeline stage name`, 2026-08-01 실측). 선택지가 아니다. 기각.
...
- 정확도 요구가 더 높아질 때 → `rerank-2.5` 2단계 추가 (Phase 밖 옵션). ⚠️ Atlas 네이티브 `$rerank` 는 **MongoDB 8.3+ 필요**하고 현재 클러스터(8.0.29)에서 거부된다 — 개정 블록 4번. 클러스터 업그레이드가 선행 조건이거나, 애플리케이션 레이어에서 Voyage API 를 직접 호출해야 한다
- `date` 정규화가 완료되면 → 개정 블록 5번의 범위 필터 경고 해제
