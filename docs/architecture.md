...
│   ├── cli.py                  ← notices 서브커맨드 (--once, --all, --source, --pages, --delay)
...
│   ├── config/
│   │   ├── loader.py           ← sources.json 로드 + 셀렉터 검증
│   │   └── sources.json    ← 학과 설정 (SSOT)
│   └── strategies/
...
| `python -m skkuverse_crawler notices --once --source skku-main --pages 3` | 단일 학과, 최대 3페이지 |
