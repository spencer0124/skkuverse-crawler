...
BeautifulSoup이 이를 파싱할 때 `<head>` 는 무시되고 `<body>` 내용이 DOM에 포함된다. 파싱 시:

```python
# 방법 1: fr-view 전체를 가져온 후 style 태그 제거
content_el = soup.select_one("div.board-view-content-wrap div.fr-view")
for tag in content_el.find_all(["style", "head"]):
    tag.decompose()
content = str(content_el).strip()

# 방법 2: body#dext_body가 있으면 그 내부만 추출
body_el = content_el.select_one("body#dext_body")
content = str(body_el) if body_el else str(content_el)
```
...
# 2페이지: pg=2
# N페이지: pg=N
url = f"{base_url}?bcode=nt&pg={page}"
```
...
이 사이트는 **EUC-KR** 인코딩을 사용한다. httpx의 기본 응답 디코딩은 UTF-8을 가정하므로, EUC-KR 페이지를 fetch하면 한글이 깨진다.

### 해결

httpx에서 바이트로 받아서 EUC-KR로 디코딩:
